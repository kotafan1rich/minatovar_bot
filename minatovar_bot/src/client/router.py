...
async def start(message: types.Message, state: FSMContext, referrer_id: Optional[int]):
    ...
    if referrer_id:
        await bot.send_message(referrer_id, MessageClient.NEW_REFERRAL)
        await bot.send_message(user_id, MessageClient.U_ARE_REFERRAL)

    await bot.send_message(
        user_id, MessageClient.START, reply_markup=types.ReplyKeyboardRemove()
    )
    await bot.send_message(
        user_id,
        MessageClient.MAIN_MENU,
...
async def back_to_menu(call: types.CallbackQuery):
    await call.message.edit_text(
        text=MessageClient.MAIN_MENU, reply_markup=ClientKeyboards.main_menu_inline_kb()
    )
...
async def promos(call: types.CallbackQuery, db_session: AsyncSession):
    ...
    if all_promos:
        await bot.send_message(
            ...
            text=MessageClient.get_promos(all_promos),
            reply_markup=ClientKeyboards.close_inline(),
        )
    else:
        await bot.send_message(
            chat_id=call.from_user.id,
            text=MessageClient.NO_PROMOS,
            reply_markup=ClientKeyboards.close_inline(),
        )
...
async def send_shoes_price(message: types.Message, state: FSMContext, db_session):
    ...
    if message.text and message.text.isdigit() and int(message.text) > 0:
        ...
        if delivery_price and current_rate:
            ...
            text = MessageClient.send_price_mes(res_price_rub)
            await bot.send_message(user_id, text)
        else:
            await bot.send_message(user_id, MessageClient.BOT_IS_UNVAILABLE)
        await bot.send_message(
            chat_id=user_id,
            text=MessageClient.MAIN_MENU,
            reply_markup=ClientKeyboards.main_menu_inline_kb(),
        )
        await state.clear()
    else:
        await bot.send_message(user_id, MessageClient.NOT_DIGIT_ERROR)
...
async def get_current_rate(call: types.CallbackQuery, db_session):
    ...
    if current_rate := await SettingsDAL(db_session).get_param("current_rate"):
        await bot.send_message(
            call.from_user.id,
            MessageClient.send_current_rate_mes(current_rate),
            reply_markup=ClientKeyboards.close_inline(),
        )
    else:
        await bot.send_message(
            call.from_user.id,
            MessageClient.BOT_IS_UNVAILABLE,
            reply_markup=ClientKeyboards.main_menu_inline_kb(),
        )
...
async def get_refferal_link(call: types.CallbackQuery):
    ...
    await call.message.edit_text(
        text=MessageClient.refferal_link(
            bot_username=info_bot.username, user_id=call.from_user.id
        ),
        reply_markup=ClientKeyboards.close_inline(),
    )
...
async def get_my_referrals(call: types.CallbackQuery, db_session: AsyncSession):
    ...
    actives = await get_active_referrals(
        user_referrals=referrals, db_session=db_session
    )
    await call.message.edit_text(
        text=MessageClient.count_referrals(referrals, actives),
        reply_markup=ClientKeyboards.close_inline(),
    )
